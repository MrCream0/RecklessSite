import {
  applesaurus,
  headshot,
  mousepad,
  hoodie,
  tumbler,
  tiktok,
  discord,
  twitter,
  phantom,
  turtle,
  mayuri,
  grade,
  meliodas,
  a,
  b,
  c,
} from "../assets";

export const navLinks = [
  {
    id: "about",
    title: "About",
  },
  {
    id: "home",
    title: "Home",
  },
  {
    id: "contact",
    title: "Contact",
  },
  {
    id: "partners",
    title: "Partners",
  },
  {
    id: "socials",
    title: "Socials",
  },
  {
    id: "rules",
    title: "Rules",
  },
  {
    id: "merch",
    title: "Merch",
  }
];

const services = [
  {
    title: "Filler Stuff",
    icon: applesaurus,
  },
  {
    title: "Not Sure",
    icon: applesaurus,
  },

];

const socials = [
  {
    name: "Discord",
    icon: discord,
    link: "https://discord.gg",
  },
  {
    name: "Twitter",
    icon: twitter,
    link: "https://twitter.com/theofficialtrd?s=11&t=GI58u04S1I_SOGbGmdvA5Q",
  },
  {
    name: "tik tok",
    icon: tiktok,
    link: "https://www.tiktok.com",
  },
];

const staff = [
  {
    title: "Phantom",
    company_name: "Owner",
    icon: applesaurus,
    iconBg: "#383E56",
    date: "TRD Phantom#6132",
    points: [
      "Experienced gamer since the age of six",
      "Proud father of three",
      "Enthusiast of cars and motorcycles",
    ],
    image: phantom,
  },
  {
    title: "TRD meliodas5012",
    company_name: "Owner",
    icon: applesaurus,
    iconBg: "#383E56",
    date: "TRD meliodas5012#5122",
    points: [
      "Devoted husband and proud father",
      "Developed a passion for gaming with my first Xbox 360 at the age of 8",
      "Committed to promoting safe and enjoyable gaming experiences since the age of 16",
    ],
    image: meliodas,
  },
  {
    title: "Grade",
    company_name: "Owner/Discord ops",
    icon: applesaurus,
    iconBg: "#383E56",
    date: "Grade#0001",
    points: [
      "Avid gamer since the release of PlayStation 2",
      "Extensive experience gained from involvement in diverse gaming communities and clans over the years.",
    ],
    image: grade,
  },
  {
    title: "TRD Turtle",
    company_name: "Director of operations",
    icon: applesaurus,
    iconBg: "#383E56",
    date: "TRD Turtle#0128",
    points: [
      "Lifelong gamer",
      "Involved in gaming community/clan for over a decade",
      "Dedicated gamer and proud father, striking a balance between gaming and family life",
    ],
    image: turtle,
  },
  {
    title: "TRD Mayuri",
    company_name: "Director of Operations for Clan Ops",
    icon: applesaurus,
    iconBg: "#383E56",
    date: "TRD Mayuri#5800",
    points: [
      "A seasoned gamer with over a decade of experience",
      "Dedicated father of two children",
      "Enjoys outdoor activities and cherishes quality time with family",
    ],
    image: mayuri,
  },
];

const testimonials = [
  {
    testimonial:
      "Large Mousepad",
    name: "",
    designation: "Mousepad",
    company: "TRD",
    image: mousepad,
  },
  {
    testimonial:
      "Tumbler",
    name: "",
    designation: "mousepad",
    company: "TRD",
    image: tumbler,
  },
  {
    testimonial:
      "Hoodie",
    name: "",
    designation: "hoodie",
    company: "TRD",
    image: hoodie,
  },
];

const projects = [
  {
    name: "Toxic Gaming",
    description:
      "Heres a desc for Toxic Gaming",
    tags: [
      {
        name: "TGK",
        color: "blue-text-gradient",
      },
      {
        name: "Toxic",
        color: "green-text-gradient",
      },
      {
        name: "Gaming",
        color: "pink-text-gradient",
      },
    ],
    image: a,
    source_code_link: "https://toxicgaming.gg/",
  },
  {
    name: "Heres partner 2",
    description:
      "lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud",
    tags: [
      {
        name: "Oompa",
        color: "blue-text-gradient",
      },
      {
        name: "loopy",
        color: "green-text-gradient",
      },
      {
        name: "doopity",
        color: "pink-text-gradient",
      },
    ],
    image: b,
    source_code_link: "https://github.com/",
  },
  {
    name: "Partner 3",
    description:
      "lorems ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud",
    tags: [
      {
        name: "Why",
        color: "blue-text-gradient",
      },
      {
        name: "Doi",
        color: "green-text-gradient",
      },
      {
        name: "Exist",
        color: "pink-text-gradient",
      },
    ],
    image: c,
    source_code_link: "https//Google.com/",
  },
];

export { services, socials, staff, testimonials, projects };